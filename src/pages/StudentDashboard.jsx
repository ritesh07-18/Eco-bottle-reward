...
import { Award, CupSoda, Gift, Recycle } from 'lucide-react';
import { BottleChart, PointsChart } from '../components/charts/EcoCharts';
import { EmptyState } from '../components/ui/EmptyState';
import { StatCard } from '../components/ui/StatCard';
import { Badge } from '../components/ui/Badge';
import { useStudentData } from '../hooks/useStudentData';
import { CO2_PER_BOTTLE_KG, bottlePointConfig } from '../lib/constants';
import { formatDateTime, isExpired } from '../lib/utils';

export default function StudentDashboard() {
  const { profile, bottles = [], coupons = [], monthly = [], pointRules = [], loading, error } = useStudentData();
  const activeCoupons = coupons.filter((coupon) => coupon.status === 'ACTIVE' && !isExpired(coupon.expiry_time)).length;
  const hasBottleData = bottles.length > 0;
  const co2Saved = (bottles.length * CO2_PER_BOTTLE_KG).toFixed(2);
  const rules = pointRules.length > 0 ? pointRules : Object.entries(bottlePointConfig).map(([bottle_type, points]) => ({ bottle_type, points }));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard icon={Recycle} label="CO2 Saved" value={`${co2Saved} kg`} helper="Estimated avoided emissions" />
      </div>
        ...
        <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              Each verified bottle type earns a fixed number of points. Redeem points for canteen discount coupons.
            </p>
          </div>
          <Link to="/redeem" className="btn-secondary h-10">
            Redeem points
          </Link>
        </div>
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          {rules.map((rule) => (
            <div key={rule.bottle_type} className="flex items-center justify-between rounded-lg border border-slate-100 bg-slate-50 p-4 dark:border-white/10 dark:bg-slate-950">
              <div>
                <p className="text-sm text-slate-500 dark:text-slate-400">Bottle Type {rule.bottle_type}</p>
                <p className="mt-1 text-2xl font-bold">{rule.points} pts</p>
              </div>
              <Badge tone="green">per bottle</Badge>
            </div>
          ))}
        </div>
      </section>
