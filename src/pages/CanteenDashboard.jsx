...
import { CheckCircle2, ReceiptText, Search, XCircle } from 'lucide-react';
...
export default function CanteenDashboard() {
  ...
  async function handleVerify() {
    ...
    try {
      ...
      setResult(data);
      if (data.coupon) {
        setCoupon((current) => ({ ...data.coupon, profiles: current?.profiles }));
      }
    } catch (err) {
      setResult({ valid: false, message: err.message });
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-end">
        <Link to="/canteen/transactions" className="btn-secondary h-10">
          <ReceiptText size={17} />
          View transactions
        </Link>
      </div>
