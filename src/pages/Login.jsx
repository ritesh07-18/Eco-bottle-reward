...
export default function Login() {
  ...
  async function handleSubmit(event) {
    ...
    setError('');
    try {
      const data = await signIn(resolveLoginEmail(identifier), password);
      if (!data.session) {
        setError('Please confirm your email address before logging in.');
        return;
      }
      navigate(data.profile?.role === 'canteen' ? '/canteen/dashboard' : '/dashboard');
    } catch (err) {
  ...
  return (
    <AuthPage>
      <form onSubmit={handleSubmit} className="surface mx-auto w-full max-w-md rounded-2xl p-6">
        ...
        <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Login with your phone number or email.</p>
        ...
        <div className="mt-6 space-y-4">
          <div>
            ...
            <input className="input mt-1.5" id="identifier" type="text" inputMode="tel" placeholder="9876543210" value={identifier} onChange={(e) => setIdentifier(e.target.value)} required />
          </div>
          <div>
