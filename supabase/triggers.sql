...
begin
  ...
  select discount into configured_discount
  from public.redemption_options
  where points = p_points_used;

  if configured_discount is null then
  ...
  values (p_user_id, new_code, p_points_used, configured_discount, 'ACTIVE', now() + interval '24 hours')
  returning * into created_coupon;

  return jsonb_build_object(
    'id', created_coupon.id,
    'user_id', created_coupon.user_id,
    'coupon_code', created_coupon.coupon_code,
    'points_used', created_coupon.points_used,
    'discount_value', created_coupon.discount_value,
    'status', created_coupon.status,
    'expiry_time', created_coupon.expiry_time,
    'created_at', created_coupon.created_at
  );
end;
$$;
...
revoke execute on function public.redeem_coupon(uuid, integer) from public, anon;
